...
\node[ucase] (uc3) at (6,7) {Mendistribusikan Likuiditas};
...
\node[font=\footnotesize\bfseries] at (5,7.2) {Operasi Wholesale};

\node[ucase] (w1) at (5,6) {Menerbitkan Digital Rupiah};
\node[ucase] (w2) at (5,5) {Mendistribusikan Likuiditas};
\node[ucase] (w3) at (5,4) {Mengelola Siklus Hidup Peserta};
\node[ucase] (w4) at (5,3) {Mengelola Kebijakan Limit};
\node[ucase] (w6) at (5,1) {Laporan \& Metrik};
...
\draw[uassoc] (sv) -- (w4);
\draw[uassoc] (sv) -- (w6);
\draw[udep] (w2) -- (w1) node[midway, above, sloped] {$\langle\langle$include$\rangle\rangle$};
\end{tikzpicture}%
}
\caption{Diagram use case operasi wholesale}
\label{fig:uc02}
\end{figure}
...
\node[ucase] (r2) at (5,5) {Mentransfer Saldo};
\node[ucase] (r3) at (5,4) {Transfer Antarpelanggan};
\node[ucase] (r4) at (5,3) {Transfer ke Pedagang};
\draw[uassoc] (pr) -- (r1);
\draw[uassoc] (pr) -- (r2);
\draw[uassoc] (pr) -- (r4);
\draw[uassoc] (m) -- (r3);
\draw[udep] (r4) -- (r3) node[midway, above, sloped] {$\langle\langle$include$\rangle\rangle$};
\end{tikzpicture}%
}
