% Diagram Entity-Relationship basis data luring (off-chain) PostgreSQL.
% Sumber kebenaran skema: backend/services/postgres_store.go:51-115.
% Di-input dari BAB III (Perancangan Basis Data Luring).
...
\caption{Diagram \textit{Entity-Relationship} basis data luring (\textit{off-chain}) pada PostgreSQL. Tabel \texttt{auth\_users} berdiri sendiri untuk autentikasi portal, sedangkan \texttt{kyc\_profiles} menjadi pusat relasi profil KYC, riwayat pemeriksaan penyedia, dan jejak audit.}
\label{fig:erd-offchain}
\end{figure}
